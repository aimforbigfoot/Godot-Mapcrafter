...
func _input(event: InputEvent) -> void:
	...
	if Input.is_action_just_pressed("step"):
		count += 1
		#sot = mgh.drawCircle(Vector2( randi_range(0,WIDTH) , randi_range(0,HEIGHT) ), 5, mgh.floorTile, sot)
		#sot = mgh.drawSquareEveryNthTiles( 3, 5, 2, mgh.TILES.FLOOR, sot )
		sot = mgh.applyFastPerlinNoise(1.5, 0.01, mgh.floorTile, sot)
		#if count <= 3:
